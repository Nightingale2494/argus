...
class ComplianceEngine:
    ...
    def evaluate(
        ...
        if facts_with_val and not verified_with_val:
            ...
            # A claimed scalar registration ID or client/bidder-controlled document_type label alone
            # cannot silently pass without authoritative external verification or human review.
            if getattr(rule, "requires_verification", False) is True:
                if are_fields_equivalent(rule.field, "gem.seller_id") or str(rule.field).startswith("gem."):
                    # In the absence of an authorized verification provider or trusted cryptographic
                    # certificate verification, all unverified documentary claims route fail-safe to human review.
                    return RuleEvaluationRead(
                        id=eval_id,
                        bidder_id=context.get("bidder_id", "UNKNOWN_BIDDER"),
                        requirement_id=rule.id,
